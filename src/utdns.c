...
/*! This function opens a UDP socket on loopback addresses (127.0.0.1 and ::1) of the
...
static int init_udp_socket(int family, int port)
{
	...
	{
	case AF_INET6:
		inet_pton(AF_INET6, "::1", &((struct sockaddr_in6*) &sock_addr)->sin6_addr.s6_addr);
		((struct sockaddr_in6*) &sock_addr)->sin6_port = htons(port);
		len = sizeof(struct sockaddr_in6);
		break;

	case AF_INET:
		((struct sockaddr_in*) &sock_addr)->sin_addr.s_addr = inet_addr("127.0.0.1");
